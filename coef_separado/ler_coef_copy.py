...
#            df_alarmes = pd.read_sql_query(query_alarmes, cnx)
    ...
    #            df_nome_alarmes = pd.read_sql_query(query_nome_alarmes, cnx)
                df_nome_alarmes = pd.read_sql_query(query_nome_alarmes, connection)
            #    cursor.execute(query_nome_alarmes)
            #    df_nome_alarmes = cursor.fetchall()    
        ...
        #        if ratio >= 1.0:
            #        print(f"{data} - Valor {ratio} é maior que 1.0")
