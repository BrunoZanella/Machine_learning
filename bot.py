...
bot = Bot(token=TOKEN)
#dp = Dispatcher(bot)
dp = Dispatcher(bot, storage=MemoryStorage())  # Adicione MemoryStorage aqui
...
async def id_chat_grupo():
    # Busca o ID do grupo no banco de dados
...
async def id_chat_usuario(username):
    # Busca o ID do chat do usuário no banco de dados
...
async def teste_menu(message: types.Message):
    ...
    # Buscar o usuário na tabela usuarios_telegram pelo id do chat
    cursor.execute("SELECT usuario FROM usuarios_telegram WHERE id_telegram = %s", (chat_id,))
    result = cursor.fetchone()
    print(result)

    # Se encontramos um resultado, pegamos o nome do usuário
    if result is not None:
        username = result[0]
        print(username)
        
        # Converter datetime.datetime para timestamp Unix
        timestamp = int(time.mktime(message.date.timetuple()))
        
        # Criar uma nova mensagem com o nome do usuário como texto
        ...
        # Chamar a função enviar_previsao_valor_equipamento com a nova mensagem e o nome do usuário
...
class Form(StatesGroup):
    ...
    usuario = State()
    ask_supervisor_name = State()  # Novo estado para perguntar o nome do supervisor
...
async def process_usuario_callback(query: CallbackQuery, state: FSMContext):
    # Obtém o nome_telegram do usuário clicado
    nome_telegram = query.data

    # Imprime o nome_telegram do usuário clicado
    print(f"Clicou no usuário com nome_telegram: {nome_telegram}")

    # Responda ao usuário com o nome_telegram do usuário clicado
    await query.answer(f"Você escolheu o usuário: {nome_telegram}")

    # Salva o nome_telegram do usuário selecionado no estado do FSM
    await state.update_data(selected_user_nome_telegram=nome_telegram)

    # Pergunta o nome do supervisor
...
async def process_supervisor_name(message: types.Message, state: FSMContext):
    ...
    # Obtém o 'codigo' do usuário atualizado
    ...
    if codigo_result:
        ...
        # Atualiza a tabela 'usuarios_telegram' com o 'codigo' obtido
    ...
    # Responde ao grupo com o nome do supervisor inserido
...
async def callback_fazer_previsao_usina_menu(query: CallbackQuery):
    try:
        # Obtém o código da usina a partir da callback data
        cod_usina = int(query.data.split('_')[3])

        # Chama a função enviar_previsao_valor_usina_menu com o código da usina
...
async def button(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    # Remove a mensagem com os botões após a seleção
...
async def process_cod_usina(message: types.Message, state: FSMContext):
    ...
    if user_input.isdigit():
        ...
        if usina_result:
            ...
            # Consulta na tabela 'equipamentos'
            ...
            # Cria o teclado de marcação inline com o botão "Fazer Previsão"
            ...
            # Cria a mensagem de resposta com as informações e o teclado inline
            response_message = (
            ...
            # Responde ao usuário com as informações e o teclado inline
...
async def process_cod_equipamento(message: types.Message, state: FSMContext):
    ...
    chat_id = await id_chat_grupo()
    # Remove o '/' do início da mensagem, se houver
    user_input = message.text.lstrip('/')
    ...
    if user_input.isdigit():
        ...
        if equipamento_result:
            ...
            if usina_result:
                ...
                # Cria a mensagem de resposta com as informações obtidas
                response_message = (
                ...
                # Enviar a mensagem com as informações do equipamento e o botão "Fazer Previsão"
                await bot.send_message(chat_id, response_message, reply_markup=keyboard)
...
async def process_usuario(message: types.Message, state: FSMContext):
    ...
    # Consulta na tabela 'usuarios_telegram' para usuários com valor 0
    ...
    if usuarios_result:
        # Cria um teclado de marcação inline
        keyboard = InlineKeyboardMarkup(row_width=1)
...
async def enviar_boas_vindas(message: types.Message):
    ...
    #            await message.reply("Olá! Qual é o seu usuário no supervisório da BRG?\nhttps://supervisorio.brggeradores.com.br/beta/index.php")
...
async def send_welcome(message: types.Message):
    ...
    if chat_type in ['group', 'supergroup']:
        ...
        if result is None:
            ...
            if primeiro_acesso == '1':
                await message.reply("Olá! Qual é o seu usuário no supervisório da BRG?\nhttps://supervisorio.brggeradores.com.br/beta/index.php")
                cursor.execute("UPDATE usuarios_telegram SET primeiro_acesso = '0' WHERE id_telegram = %s", (chat_id,))
            else:
                await message.reply(f"Bem-vindo de volta, {result['usuario']}!\nDigite /menu para opções")
            cnx.commit()
    else:
        await message.reply("Olá! Qual é o seu usuário no supervisório da BRG?\nhttps://supervisorio.brggeradores.com.br/beta/index.php")
...
#async def boas_vindas(message: types.Message):
    user_input = message.text.lstrip('/')
    username = user_input
#    username = message.text
    ...
    if result is not None:
        ...
        if primeiro_acesso == 1:
            ...
            # Ajuste na chamada da função para incluir o id_telegram, se disponível
            if id_telegram:
...
#        await enviar_cod_equipamento_usuario(message)
        await enviar_previsao_valor_equipamento(message)

#    else:
#        await message.reply("Usuário não encontrado.")
    else:
        # Consulta na tabela 'usuarios_telegram' para usuários com o mesmo 'nome_telegram'
        ...
        if result is not None:
            ...
            if chat_type in ['group', 'supergroup']:
                ...
                result = cursor.fetchone()
                if result is None:
                    # Se um usuário com o mesmo 'nome_telegram' for encontrado, atualize o registro
            ...
            # Se nenhum usuário com o mesmo 'nome_telegram' for encontrado, insira um novo registro
            ...
            cursor.execute(query, values)
            cnx.commit()
        #    await message.reply("Nome do usuário no Telegram salvo com sucesso!")
...
#        print(f'{cod_equipamento} - Coeficiente Existente = {coeficiente_existente}, Intercepto Existente = {intercepto_existente}, Acuracia = {acuracia_existente}')
...
def fazer_previsao_sempre(valores_atuais, coeficiente, intercepto, cod_equipamento_resultado):
    ...
    # Busque os valores atuais do cod_campo 114
    ...
    # Busque os valores atuais do cod_campo 3
    ...
    # If valores_atuais_3 is None or contains 0, return False
    if valores_atuais_3 is None or 0 in valores_atuais_3:
#        print('cod_equipamento_resultado',cod_equipamento_resultado,'valores_atuais_3',valores_atuais_3)
...
#        elif calculo < limite_menos:
        if calculo < limite_porcentagem_menos:
            contagem_abaixo_do_limite +=1
            print('ABAIXO')

#        if abs(calculo) > limite_mais:
        elif abs(calculo) > limite_porcentagem_mais:
    ...
    return previsoes, contagem_acima_do_limite > 4 or contagem_abaixo_do_limite > 4
        ...
        for cod_equipamento in cod_equipamentos:
            ...
            try:
                ...
                for cod in cod_campo_especificados:
                    ...
                    # Selecione a data_cadastro do cod_campo específico
                    ...
                    if data_cadastro is not None:
                        ...
                        # Verifique se a data_cadastro já existe na tabela leituras_consecutivas
            ...
            try:
                ...
                for resultado in resultados_valores:
                    ...
                    if cod_campo == 3:
                        ...
                        coeficiente = 0.0
                        intercepto = 0.0
                    #    previsoes = fazer_previsao([valor_1, valor_2, valor_3, valor_4, valor_5], coeficiente, intercepto, cod_equipamento_resultado)
...
async def enviar_previsao_valor_equipamento_alerta(cod_equipamentos, tabelas, cod_campo_especificados):
    ...
    cod_equipamentos_tuple = ', '.join(map(str, cod_equipamentos))
    query = f"SELECT cod_equipamento, valor_1, valor_2, valor_3, valor_4, valor_5 FROM leituras_consecutivas WHERE cod_equipamento IN ({cod_equipamentos_tuple}) AND cod_campo IN (3, 114)"
    cursor.execute(query)

    valores_atuais_all = cursor.fetchall()
#    valores_atuais_dict = {cod_equipamento: valores for cod_equipamento, *valores in valores_atuais_all}
    valores_atuais_dict = {cod_equipamento: (valor_1, valor_2, valor_3, valor_4, valor_5) for cod_equipamento, valor_1, valor_2, valor_3, valor_4, valor_5 in valores_atuais_all}


    while True:
        alertas_por_usina = {}
        for cod_equipamento in cod_equipamentos:
            valores_atuais = valores_atuais_dict.get(cod_equipamento)

            if 0 in valores_atuais:
                continue
        ...
        for cod_usina, mensagens in alertas_por_usina.items():
            ...
            for cod_usuario_tuple in cod_usuarios:
                ...
                cursor.execute("SELECT id_telegram, usuario, nome_supervisorio FROM usuarios_telegram WHERE cod_usuario = %s", (cod_usuario,))
                result = cursor.fetchone()
                if result is not None:
                    id_telegram, nome_usuario, nome_supervisorio = result
                    try:
                        await bot.send_message(id_telegram, mensagem_final)
                    except BotBlocked:
                        if cod_usuario not in usuarios_bloqueados:
                            id_grupo = await id_chat_grupo()
                            if id_grupo is not None:
                                await bot.send_message(id_grupo, f"O bot foi bloqueado por {nome_supervisorio} \n(cod do usuario {cod_usuario})")
    ...
    while True:
        ...
        for cod_equipamento in cod_equipamentos:
            ...
            if alerta:
                ...
                # Verifique se um alerta já foi enviado para este equipamento
                if cod_equipamento not in alertas_enviados:
                    ...
                    if result is not None:
                        ...
                        # Adicione o equipamento ao conjunto de alertas enviados
                        alertas_enviados.add(cod_equipamento)
                        print('equipamento adicionado')
            else:
                if cod_equipamento in alertas_enviados:
                    # Se não houver alerta, remova o equipamento do conjunto de alertas enviados
...
ultimos_alertas = {}  # novo dicionário para armazenar os últimos 5 alertas para cada equipamento
...
async def enviar_previsao_valor_equipamento_alerta(cod_equipamentos, tabelas, cod_campo_especificados):
    ...
    while True:
        ...
        for cod_equipamento in cod_equipamentos:
            ...
            ultimos_alertas[cod_equipamento].append(int(alerta))  # adicione o alerta (como 0 ou 1) aos últimos alertas
            ...
            if len(ultimos_alertas[cod_equipamento]) == 5:
                ...
                print(f'{cod_equipamento} ultimos_alertas[cod_equipamento]',ultimos_alertas[cod_equipamento])
    ...
    #        mensagem_final = f"ALERTA! \n\nUsina: {cod_usina} - {nome_usina} \n\n" + ''.join(mensagens)
        #    mensagem_final = f"ALERTA! \n\nUsina: {cod_usina} - {nome_usina} \n\n https://supervisorio.brggeradores.com.br/beta/detalhesusinaover.php?codUsina={cod_usina}\n\n" + ''.join(mensagens)
...
ultimos_valores = {}
alertas_enviados = set()  # novo conjunto para controlar os alertas enviados
    ...
    while True:
        ...
        for cod_equipamento in cod_equipamentos:
            ...
            if len(ultimos_valores[cod_equipamento]) == 5:
                ...
                print(f'{cod_equipamento} ultimos_valores[cod_equipamento] - ',ultimos_valores[cod_equipamento])
                print('media - ',media)
                if media >= 80 and cod_equipamento not in alertas_enviados:
    ...
    if result is not None:
        ...
        if primeiro_acesso == 1:
            ...
            for cod_usina_tuple in cod_usinas:
                ...
                # Converter a lista de equipamentos em uma string
                equipamentos_str = ', '.join(equipamentos_lista)

                # Adicionar a usina e os equipamentos à mensagem
                mensagem += f"Usina: {cod_usina} - Equipamentos: {equipamentos_str}\n"

            if len(mensagem) > 4096:
                # Se a mensagem for muito longa, divida-a em várias partes
        ...
        #            await bot.send_message(chat_id, parte)
                ...
                # Se a mensagem não for muito longa, envie-a como está
...
async def enviar_previsao_valor_equipamento(message: types.Message, id_telegram=None):
    ...
    if result is not None:
        ...
        if primeiro_acesso == 1:
            ...
            #            previsoes = fazer_previsao(valores_atuais, coeficiente_existente, intercepto_existente, int(equipamento))
            ...
            # Verificar se a mensagem é '/teste'
    ...
    if result is not None:
        ...
        if primeiro_acesso == 1:
            ...
            for cod_usina_tuple in cod_usinas:
                ...
                # Obter os valores atuais para todos os equipamentos de uma vez
                cod_equipamentos_tuple = ', '.join(map(str, equipamentos_lista))
                query = f"SELECT cod_equipamento, valor_1, valor_2, valor_3, valor_4, valor_5 FROM leituras_consecutivas WHERE cod_equipamento IN ({cod_equipamentos_tuple}) AND cod_campo IN (3, 114)"
                cursor.execute(query)
                ...
                for equipamento in equipamentos_lista:
                    valores_atuais = valores_atuais_dict.get(int(equipamento))
...
async def enviar_previsao_valor_usina_menu(cod_usina, chat_id=None):
    ...
    # Para cada equipamento, faça a previsão e adicione à mensagem
    for equipamento in equipamentos_lista:
        # Consultar os valores atuais para o equipamento
    ...
    #    previsoes = fazer_previsao(valores_atuais, coeficiente_existente, intercepto_existente, equipamento)
        previsoes, alerta = fazer_previsao(valores_atuais, coeficiente_existente, intercepto_existente, int(equipamento))

        cursor.execute("SELECT valor_1, valor_2, valor_3, valor_4, valor_5 FROM leituras_consecutivas WHERE cod_campo = 114 AND cod_equipamento = %s", (int(equipamento),))
        valores_atuais_114 = cursor.fetchone()

        # Adicionar os valores atuais à mensagem
        valores_atuais_str = ', '.join(map(str, valores_atuais_114))

        equipamentos_str = f'\nValores Atuais: {valores_atuais_str} \nValores Previstos: {previsoes}\n{alerta}'

        # Adicionar a usina, o equipamento, os valores atuais, o valor real e as previsões à mensagem
        mensagem += f"Equipamento: {equipamento} - ({nome_equipamento}): {equipamentos_str}\n\n"

    # Verifica se chat_id é fornecido (se não, usa chat_id padrão)
...
async def enviar_previsao_valor_usina_menu(cod_usina, chat_id=None):
    ...
    placeholders = ', '.join(['%s'] * len(codigos_GMG))
    query = f"SELECT codigo FROM equipamentos WHERE cod_usina = %s AND cod_tipo_equipamento IN ({placeholders})"
    cursor.execute(query, [cod_usina] + list(codigos_GMG))
    ...
    for equipamento in equipamentos_lista:
        ...
        if valores_atuais:
            ...
            # Buscar o valor real do cod_campo 114 para o equipamento
            cursor.execute("SELECT valor FROM leituras WHERE cod_equipamento = %s AND cod_campo = 114 ORDER BY data_cadastro DESC LIMIT 1", (int(equipamento),))
            valor_real = cursor.fetchone()
...
async def enviar_previsao_valor_equipamento_menu(chat_id, cod_equipamento):
    # Realizar as consultas necessárias e fazer a previsão para o cod_equipamento
    ...
    #        previsoes = fazer_previsao(valores_atuais, coeficiente_existente, intercepto_existente, cod_equipamento)
            previsoes, alerta = fazer_previsao(valores_atuais, coeficiente_existente, intercepto_existente, int(cod_equipamento))

            # Criar a mensagem de resposta com as previsões
            response_message = (
...
async def enviar_previsao_valor_equipamento_menu(chat_id, cod_equipamento):
    # Realizar as consultas necessárias e fazer a previsão para o cod_equipamento
    cursor.execute("SELECT nome, cod_usina FROM equipamentos WHERE codigo = %s", (cod_equipamento,))
    equipamento_info = cursor.fetchone()

    if equipamento_info:
        nome_equipamento, cod_usina = equipamento_info

        # Obter os valores atuais para o equipamento
        query = f"SELECT cod_equipamento, valor_1, valor_2, valor_3, valor_4, valor_5 FROM leituras_consecutivas WHERE cod_equipamento = {cod_equipamento} AND cod_campo IN (3, 114)"
        cursor.execute(query)
...
if __name__ == '__main__':
    ...
    # Crie o menu e use-o para enviar uma mensagem, por exemplo
